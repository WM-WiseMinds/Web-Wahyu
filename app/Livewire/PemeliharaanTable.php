...
final class PemeliharaanTable extends PowerGridComponent
{
    use WithExport;
    use Toastable;
    ...
    public function setUp(): array
    {
        ...
        $setUp = [
            ...
            Footer::make()
                ...
                ->showRecordCount(),
            Detail::make()
                ->view('details.pemeliharaan-detail')
                ->showCollapseIcon(),
        ];

        if (auth()->user()->can('export')) {
            $setUp[] = Exportable::make('pemeliharaan')
                ->striped()
    ...
    {
        return Pemeliharaan::query()->with('mobil');
    }
    ...
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ->add('id')
            ->add('mobil_id')
            ->add('nama_mobil', fn ($row) => $row->mobil->nama)
            ->add('keterangan')
    ...
    public function actions(Pemeliharaan $row): array
    {
        $actions = [];
        ...
        if (auth()->user()->can('update')) {
            $actions[] =
                ...
                ->openModal('pemeliharaan-form', ['rowId' => $row->id]);
        }
    ...
    public function exportPdf()
    {
        ...
        // Membuat file pdf
        $pdf = Pdf::loadView('pdf.pemeliharaan', ['datasource' => $datasource]);
        // Menyimpan file pdf ke folder pdf
        $pdf->save($path . '/pemeliharaan.pdf');
        // Menampilkan file pdf
        return response()->download($path . '/pemeliharaan.pdf');
    }

    // Function to delete data
    public function delete($rowId)
    {
        $pemeliharaan = Pemeliharaan::findOrFail($rowId);
        $pemeliharaan->delete();

        $this->success('Pemeliharaan berhasil dihapus');
    }
}
