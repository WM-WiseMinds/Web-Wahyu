...
final class PelangganTable extends PowerGridComponent
{
    ...
    {
        return Pelanggan::query()->with('user');
    }
    ...
    public function actions(Pelanggan $row): array
    {
        ...
        if (auth()->user()->can('update')) {
            $actions[] =
                ...
                ->openModal('pelanggan-form', ['rowId' => $row->id]);
        }
    ...
    public function exportPdf()
    {
        ...
        // Membuat file pdf
        $pdf = Pdf::loadView('pdf.pelanggan', ['datasource' => $datasource]);
        // Menyimpan file pdf ke folder pdf
        $pdf->save($path . '/pelanggan.pdf');
        // Menampilkan file pdf
        return response()->download($path . '/pelanggan.pdf');
    }

    // Function to delete data
    public function delete($rowId)
    {
        $pelanggan = Pelanggan::findOrFail($rowId);
        $pelanggan->delete();
