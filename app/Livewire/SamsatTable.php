...
final class SamsatTable extends PowerGridComponent
{
    use WithExport;
    use Toastable;
    ...
    public function setUp(): array
    {
        ...
        $setUp = [
            ...
            Footer::make()
                ...
                ->showRecordCount(),
            Detail::make()
                ->view('details.samsat-detail')
                ->showCollapseIcon(),
        ];

        if (auth()->user()->can('export')) {
            $setUp[] = Exportable::make('users')
                ->striped()
    ...
    {
        return Samsat::query()->with('mobil');
    }
    ...
    public function actions(Samsat $row): array
    {
        $actions = [];
        ...
        if (auth()->user()->can('update')) {
            $actions[] =
                ...
                ->openModal('samsat-form', ['rowId' => $row->id]);
        }
    ...
    public function exportPdf()
    {
        ...
        // Membuat file pdf
        $pdf = Pdf::loadView('pdf.samsat', ['datasource' => $datasource]);
        // Menyimpan file pdf ke folder pdf
        $pdf->save($path . '/samsat.pdf');
        // Menampilkan file pdf
        return response()->download($path . '/samsat.pdf');
    }

    // Function to delete data
    public function delete($rowId)
    {
        $samsat = Samsat::findOrFail($rowId);
        $samsat->delete();

        $this->success('Samsat berhasil dihapus');
    }
}
